...
import { Heart, ShoppingCart, Star, Users, ArrowRight, Sparkles, Filter } from "lucide-react";
  ...
  if (isLoading) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[80vh] space-y-4">
        ...
        <p className="text-violet-600 font-bold animate-pulse text-sm uppercase tracking-widest">Initializing Nexus Hub...</p>
      </div>
    );
  }

  if (error) return <p className="p-10 text-red-500 text-center font-semibold">Error connecting to Nexus. Please refresh.</p>;

  const filteredVideos = category === "All" ? videos : videos?.filter((v: any) => v.category === category);
      ...
      <div className="sticky top-0 z-40 w-full bg-white/80 backdrop-blur-xl border-b border-slate-200/60 shadow-sm">
        <div className="max-w-7xl mx-auto px-6 h-16 flex items-center justify-between">
          <div className="flex items-center gap-2 text-slate-500">
            ...
            <span className="text-[11px] font-bold uppercase tracking-widest">Filter By:</span>
          </div>
      ...
      <div className="relative overflow-hidden pt-16 pb-12 px-6 lg:px-10 bg-white">
        ...
        <div className="max-w-7xl mx-auto space-y-5">
          <Badge className="bg-violet-100 text-violet-700 border-none px-4 py-1 hover:bg-violet-100">
            <Sparkles size={14} className="mr-2" /> 2024 Curriculum Updated
          </Badge>
          <div className="max-w-3xl space-y-3">
            ...
            <p className="text-base text-slate-600 leading-relaxed font-medium">
              Join <span className="text-slate-900 font-bold">50,000+ students</span>. Expert-led courses designed for your career.
            </p>
          </div>
        </div>
      </div>
          ...
          {paginatedVideos?.map((video: any) => (
            ...
            >
              ...
              <div className="relative aspect-video overflow-hidden">
                ...
                />
                <button className="absolute top-3 right-3 z-10 bg-white/90 backdrop-blur-md p-1.5 rounded-lg shadow-sm hover:bg-rose-500 hover:text-white transition-all duration-300">
              ...
              <div className="p-5 space-y-3">
                ...
                <div className="flex items-center justify-between">
                  <div>
                    <p className="text-xl font-black text-slate-900">₹{video.price}</p>
                  </div>
